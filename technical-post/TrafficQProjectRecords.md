...
```
    ...
    Cell In[21], line 1
    ----> 1 kx.q.sql("select nombre as name from $1 where tipo_elem='other' limit 5", traffic_station)


    File ~\anaconda3\envs\py10\lib\site-packages\pykx\query.py:528, in SQL.__call__(self, query, *args)
        462 def __call__(self, query: str, *args: Any) -> k.Table:
        463     """Compile and run a SQL statement.
        464 
        465     Parameters:
       (...)
        526     ```
        527     """
    --> 528     return self._q('.s.sp', k.CharVector(query), args)


    File ~\anaconda3\envs\py10\lib\site-packages\pykx\embedded_q.py:142, in EmbeddedQ.__call__(self, query, wait, sync, *args)
        140 result = _keval(bytes(wrappers.CharVector(query)), *[wrappers.K(x) for x in args])
        141 if wait is None or wait:
    --> 142     return factory(result, False)
        143 return self('::', wait=True)


    File ~\anaconda3\envs\py10\lib\site-packages\pykx\_wrappers.pyx:499, in pykx._wrappers._factory()


    File ~\anaconda3\envs\py10\lib\site-packages\pykx\_wrappers.pyx:492, in pykx._wrappers.factory()
...
![png](TrafficQProjectRecords_files/TrafficQProjectRecords_132_0.png)
...
![png](TrafficQProjectRecords_files/TrafficQProjectRecords_134_0.png)
